...
// import "svg2pdf.js";
import { exportHighChart, toImageData } from '../services/hightchart';
import { useTranslation } from 'react-i18next';

const PADDING_BOT = 5;
const PAGE_MARGIN = 10;

function ExportPdfButton({ forwardingRefs, orgUnit, year }) {
    ...
    const exportPdf = async (e) => {
        ...
        try {
            ...
            const centeredText = (text, y) => {
                ...
                const xOffsetReport =
                    pdf.internal.pageSize.width / 2 - dim.w / 2;
                ...
                if (ref.current) {
                    switch (true) {
                        ...
                        case !!ref.current.chart: {
                            const imageData = await exportHighChart({
                                filename: 'chart',
                                type: 'image/png',
                                width: 0,
                                scale: 2,
                                svg: ref.current.chart.getSVG(),
                            })
                                .then((res) => res.blob())
                                .then(toImageData);
                            const width = 190;
