...
export default class DataApiClass extends BaseApiClass {
  ...
  postDe = async ({ eventId, deId, value }) => {
    return await push(
      this.baseUrl,
      this.username,
      this.password,
      `/api/events/${eventId}/${deId}`,
      {
        dataValues: [{
          dataElement: deId,
          value: value
  ...
  postAttribute = ({ tei, attributes }) =>
    push(
      this.baseUrl,
      this.username,
      this.password,
      `/api/trackedEntityInstances/${tei}`,
      attributes,
      "PUT"
    );
  ...
  getHHStatusList = ({ orgUnit, program, hhStatus}) =>
    pull(
      this.baseUrl,
      this.username,
      this.password,
      ...
      [ `skipPaging=true`,
        `orgUnit=${orgUnit}`,
        `program=${program}`,
        `fields=trackedEntity&filter=CNqaoQva9S2:eq:${hhStatus}`,
      ]
    );

  getHouseHoldList = ({ ou, program,assignedTo }) =>
    pull(
      this.baseUrl,
      this.username,
      this.password,
      ...
      [  `skipPaging=true`,
        `ou=${ou}`,
        `program=${program}`,
        `filter=AaFN0HwuzeK:eq:${assignedTo}`,
