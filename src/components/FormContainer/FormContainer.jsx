...
const FormContainer = ({ programMetadata, data: json, setIsLoading }) => {
    ...
    const [selectedYearPicker, setSelectedYearPicker] = useState(
        moment(`01/01/${maxDate}`, 'DD/MM/YYYY')
            .endOf('year')
            .format('YYYY-MM-DD')
    );
        ...
        if (profile._isDirty && profile.isPass) {
            ...
            if (profile.isNew) {
                ...
                handleAddSelectedYear(
                    moment(profile.enrollment.enrollmentDate).year()
                );
    ...
    const handleTransformEvent = (initEv) => {
        ...
        temp.dataValues = temp.dataValues
            ? initEv.dataValues.reduce((accumulator, currentDataValue) => {
                  accumulator[currentDataValue.dataElement] =
        ...
        if (lastestEvent) {
            ...
            if (dataValues && dataValues.length > 0) {
                ...
                if (cascadeData && cascadeData.length > 0) {
                    if (
                        profile.attributes['IEE2BMhfoSc'] !=
                    ...
                    if (
                        profile.attributes['IBLkiaYRRL3'] !=
    ...
    const handleAddSelectedYear = (year) => {
        ...
        if (!existedYear.includes(year)) {
            ...
            let newEv = {
                ...
                status: 'ACTIVE',
                dataValues:
                    dataValues && dataValues.length > 0 ? dataValues : [],
            };
                    ...
                    >
                        <>
                            <DatePicker
                                ...
                                onChange={(date) =>
                                    handleAddSelectedYear(moment(date).year())
                                }
                                ...
                                minDate={
                                    new Date(profile.enrollment.enrollmentDate)
                                }
                                maxDate={new Date(`${maxDate}-12-31`)}
                            />
                            {warningText && (
                                <Alert severity="error">{warningText}</Alert>
                            )}
                        </>
                    </MuiPickersUtilsProvider>
                </Dialog>
                ...
                {[1, 2].includes(activeStep) && (
                    <div className={yearSelectionContainer}>
                        ...
                        <ConfirmDialog
                            ...
                            onConfirm={(e) =>
                                handleDeleteEventConfirm(selectedDeleteEvent)
                            }
                        >
                            {t('isDeleteEvent')}
                        </ConfirmDialog>
                            ...
                            >
                                <>
                                    <DatePicker
                                        ...
                                        minDate={
                                            new Date(
                                                profile.enrollment.enrollmentDate
                                            )
                                        }
                                        maxDate={new Date(`${maxDate}-12-31`)}
                                    />
