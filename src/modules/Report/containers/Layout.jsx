        ...
        try {
            const data = await Promise.all([
                dataApi.get(`/api/analytics.json`, { paging: false }, [
                    `dimension=pe:${year}&dimension=dx:${all_dx[0]};&filter=ou:LEVEL-${selectedOrgUnit.level};${selectedOrgUnit.id}&displayProperty=NAME`,
                ]),
                getOptionDataByDeId(WATER_SOURCE_DE_ID)(
                    year,
                    selectedOrgUnit,
                    dataApi
                ),
                getOptionDataByDeId(TOILET_TYPE_DE_ID)(
                    year,
                    selectedOrgUnit,
                    dataApi
                ),
            ...
            for (let index = 1; index < all_dx.length; index++) {
                await dataApi
                    .get(`/api/analytics.json`, { paging: false }, [
                        `dimension=pe:${year}&dimension=dx:${all_dx[index]};&filter=ou:LEVEL-${selectedOrgUnit.level};${selectedOrgUnit.id}&displayProperty=NAME`,
                    ])
                    .then((res) => {
                        const { rows } = res;
