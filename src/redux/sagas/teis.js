...
import { GET_TEIS, TABLE_CHANGE_PAGE, TABLE_FILTER, TABLE_SORT, DATACOLLECTERCOUNT } from "../types/teis";
...
function* getTeis(newPayload = {}) {
  ...
  try {
    ...
    if (offlineStatus) {
      ...
      hhStatusList = {
       approved: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Approved"}),
       needsUpdate: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Needs_updates"}),
       onGoing: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Ongoing"}),
       pending: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Pending"}),
       refused: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Refused"}),
       submitted: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Submitted"}),
       synced: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Synced"}),
        noneligible: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Non-Eligible"}),
        duplicate: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Duplicate"}),
        migrated: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Migrated"}),
        other: yield call(dataApi.getHHStatusList, {orgUnit:selectedOrgUnit.id,program:programMetadata.id, hhStatus: "Other"})

      }
    }

    yield put(getTeisSucceed(instanceList));
    yield put(getHHStatusList(hhStatusList))
...
export default function* getTeisSaga() {
  ...
  yield takeLatest(DATACOLLECTERCOUNT, fetchHouseholdListSaga);
}
...
function* fetchHouseholdListSaga(action) {
  try {
    console.log("Saga started");
    AssignedCount = yield call(dataApi.getHouseHoldList, action.payload);
    console.log("API response:", AssignedCount?.trackedEntityInstances);

       yield put(AssginList(AssignedCount))
